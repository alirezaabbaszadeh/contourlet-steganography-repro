...
6. [`STATISTICAL_ANALYSIS_PLAN.md`](STATISTICAL_ANALYSIS_PLAN.md) — endpoints, paired analysis, multiplicity, and missingness.
7. [`COCO_DATA_SOURCE.md`](COCO_DATA_SOURCE.md) — frozen COCO 2017 CC BY 2.0 source and attribution policy.
...
## Implementation and CI state

Implemented and tested: format-v2, C3_NP, payload/PSNR sweeps, failure severity, frozen B1/B2, seven-method resumable dispatcher, 16-worker benchmark harness, path-independent stability builder, automatic runtime binding freeze, one-command final-plan preparation, pair-level statistics/reporting, and final-only archive tooling.

Relevant stale CI failures have been repaired. At commit `19dcf3b40140802733b6161f475d5a0b111d8b06`, all reported 5J workflows completed successfully. Runner-preflight run `31271680057` subsequently passed the new runtime-freeze and path-independent stability tests.
...
This is intentionally unbound. The final scientific plan/run ID is generated only after the actual server runtime and real stability profile are hashed and validated.
...
4. run the 16-worker benchmark and tune from measured evidence;
5. run the two-pair seven-method dry run;
...
Remote backup does not block numerical execution or resume. After the entire computation and publication package are complete:
