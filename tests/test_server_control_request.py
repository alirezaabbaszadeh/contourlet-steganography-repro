...
class ServerControlRequestTests(unittest.TestCase):
    ...
    def test_accepts_worker_count_at_44(self) -> None:
        request = parse_request(self.base("worker_benchmark", workers=44))
        self.assertEqual(request.workers, 44)

    def test_rejects_worker_count_above_44(self) -> None:
        with self.assertRaisesRegex(RequestError, "maximum worker count is 44"):
            parse_request(self.base("worker_benchmark", workers=45))
