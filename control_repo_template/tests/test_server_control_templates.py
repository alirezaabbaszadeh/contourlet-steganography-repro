...
class ServerControlTemplateTests(unittest.TestCase):
    ...
    def test_server_control_is_push_only_on_requests(self) -> None:
        ...
        self.assertIn("ctsteg-ferdowsi-8c16g", text)
    ...
    def test_private_template_contains_control_package_and_tests(self) -> None:
        for source in sorted((ROOT / "control").glob("*.py")):
            mirrored = TEMPLATE / "control" / source.name
            self.assertTrue(mirrored.is_file(), source.name)
            self.assertEqual(mirrored.read_bytes(), source.read_bytes(), source.name)
        for source in sorted((ROOT / "tests").glob("test_server_control_*.py")):
            mirrored = TEMPLATE / "tests" / source.name
            self.assertTrue(mirrored.is_file(), source.name)
            self.assertEqual(mirrored.read_bytes(), source.read_bytes(), source.name)

    def test_public_scientific_repo_has_no_self_hosted_control_workflow(self) -> None:
        self.assertFalse((ROOT / ".github/workflows/server-control.yml").exists())

    def test_control_paths_match_finalizer_outputs(self) -> None:
        payload = json.loads(self.read("config/ctsteg-control.example.json"))
        self.assertEqual(payload["plan"], "/srv/ctsteg/finalization/final-5j-bound.json")
        self.assertEqual(payload["runtime_bindings"], "/srv/ctsteg/finalization/final-5j-runtime-bindings.json")
        self.assertEqual(payload["science_ready_report"], "/srv/ctsteg/finalization/input-readiness.json")
        self.assertEqual(
