...
def _require_target_host() -> dict[str, Any]:
    cpus = os.cpu_count() or 1
    memory = _host_memory_bytes()
    if cpus < 32:
        raise WorkerTrialError(
            f"worker tuning target requires at least 32 logical CPUs; found {cpus}"
        )
    if memory is None or memory < 60 * GIB:
        found = "unknown" if memory is None else f"{memory / GIB:.2f} GiB"
        raise WorkerTrialError(
            f"worker tuning target requires approximately 64 GiB RAM; found {found}"
        )
